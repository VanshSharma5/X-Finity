...
%>
	...
	<form  style="color:darkblue;background-color:lightblue; margin-top: -50px" class="ftco-section" method="get" action="/contact/search">
		<div  class="container">
			<!-- <div class="row justify-content-center"> -->
				<!-- <div class="col-md-7 col-lg-5"> -->
					<div class="login-wrap p-5 p-md-10" style="color:darkblue;background-color:aliceblue;" >
						<div class=" d-flex align-items-center justify-content-center">
							<svg width="100px" height="100px" viewBox="-6 -6 36.00 36.00" fill="none" xmlns="http://www.w3.org/2000/svg"><g id="SVGRepo_bgCarrier" stroke-width="0"><path transform="translate(-6, -6), scale(2.25)" fill="#7ed0ec" d="M9.166.33a2.25 2.25 0 00-2.332 0l-5.25 3.182A2.25 2.25 0 00.5 5.436v5.128a2.25 2.25 0 001.084 1.924l5.25 3.182a2.25 2.25 0 002.332 0l5.25-3.182a2.25 2.25 0 001.084-1.924V5.436a2.25 2.25 0 00-1.084-1.924L9.166.33z" strokewidth="0"></path></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"> <path fill-rule="evenodd" clip-rule="evenodd" d="M1 5C1 3.34315 2.34315 2 4 2H8.43845C9.81505 2 11.015 2.93689 11.3489 4.27239L11.7808 6H13.5H20C21.6569 6 23 7.34315 23 9V11C23 11.5523 22.5523 12 22 12C21.4477 12 21 11.5523 21 11V9C21 8.44772 20.5523 8 20 8H13.5H11.7808H4C3.44772 8 3 8.44772 3 9V10V19C3 19.5523 3.44772 20 4 20H9C9.55228 20 10 20.4477 10 21C10 21.5523 9.55228 22 9 22H4C2.34315 22 1 20.6569 1 19V10V9V5ZM3 6.17071C3.31278 6.06015 3.64936 6 4 6H9.71922L9.40859 4.75746C9.2973 4.3123 8.89732 4 8.43845 4H4C3.44772 4 3 4.44772 3 5V6.17071ZM20.1716 18.7574C20.6951 17.967 21 17.0191 21 16C21 13.2386 18.7614 11 16 11C13.2386 11 11 13.2386 11 16C11 18.7614 13.2386 21 16 21C17.0191 21 17.967 20.6951 18.7574 20.1716L21.2929 22.7071C21.6834 23.0976 22.3166 23.0976 22.7071 22.7071C23.0976 22.3166 23.0976 21.6834 22.7071 21.2929L20.1716 18.7574ZM13 16C13 14.3431 14.3431 13 16 13C17.6569 13 19 14.3431 19 16C19 17.6569 17.6569 19 16 19C14.3431 19 13 17.6569 13 16Z" fill="#000000"></path> </g></svg>
		      	        </div>
		      	        <strong><h3 style="font-weight:bold" class="text-center">Search Contact</h3></strong>
	           	<div style="display:flex" class="form-group">
					<div style="flex:1">
						<select name="attribute"class="form-control rounded-left" required>
							<option value="${param.attribute}" selected hidden><%= request.getParameter("attribute")!=null?request.getParameter("attribute"):"Filter Value" %></option>
							<option value="byName">By Name</option>	
							<option value="byContactNo">By Contact No</option>
							<option value="byPinCode">By PinCode</option>
							<option value="byDealerType">By Dealer Type</option>
							<option value="byDesignation">By Designation</option>
							<option value="All">All</option>
						</select>
					</div>
					<div style="flex:2">
						<input type="text" pattern="[a-zA-Z0-9]+" name="value" value="${param.value}" class="form-control rounded-left" placeholder="value..." required>
					</div>
					<!-- <select name="sortBy"  class="form-control rounded-left" >
						<option value="${param.sortBy}" selected hidden><%= request.getParameter("sortBy")!=null?request.getParameter("sortBy"):"Sort By" %></option>
						<option value="name">By Name</option>	
						<option value="contactNo">By Contact No</option>
						<option value="pinCode">By PinCode</option>
						<option value="dealerType">By Dealer Type</option>
						<option value="designation">By Designation</option>
					</select> -->
	           	</div>
	           	<div class="form-group d-flex">
	            	<input type="number" name="page" value="${param.page}" class="form-control rounded-left" placeholder="Page No...." >
	            	<input type="number" name="size" value="${param.size}"  class="form-control rounded-left" placeholder="Max data per Page...." >
					<!-- <select name="ascending"  class="form-control rounded-left" >
						<option value="${param.ascending}" selected hidden><%= request.getParameter("ascending")!=null?request.getParameter("ascending").equals("true")?"Assending":"Decending":"Order" %></option>
						<option value="true">Assending</option>
						<option value="false">Decending</option>
					</select> -->
	           	</div>
							<div style="display:flex" class="form-group">
								<button type="submit" style="flex:2; ; margin:10px" class="btn btn-primary rounded submit px-3"><%= request.getParameter("page") != null?"Next Page":"Search Result" %></button>
							</div>
	                </div>	
<%
	if(contacts != null) {
...
<div style="margin-top:-10%" class="ftco-section table-container">
	...
	<caption>Current ${param.page}</caption>
        <thead>
            <tr>
				<th scope="col">Id</th>
			...
			<tr>
				...
				for(Contact contact: contacts) {
					...
					<tr onclick="window.location.href='/contact?id=<%= contact.getId() %>'">
						...
						<td><%=contact.getContactNoInformataion()%></td>
						<td><%=contact.getDesignation()%></td>
						<td><%=contact.getHouseNo()%></td>
						<td><%=contact.getColony()%></td>
						<td><%=contact.getCity()%></td>
						<td><%=contact.getSubDistrict()%></td>
						<td><%=contact.getPinCode()%></td>
						<td><%=contact.getState()%></td>
						<td><%=contact.getDealerType()%></td>
						<td><%=contact.getDealerVisit()%></td>
						<td><%=contact.getDealerVisitDate()%></td>
						<td><%=contact.getCompanyVisit()%></td>
						<td><%=contact.getCompanyVisitDate()%></td>
						<td><%=contact.getExistingDealer()%></td>
						<td><%=contact.getCallDetailCount()%></td>
						<td><%=contact.getDiscussion1()%></td>
						<td><%=contact.getDiscussion2()%></td>
						<td><%=contact.getDiscussion3()%></td>
						<td><%=contact.getDiscussion4()%></td>
						<td><%=contact.getDiscussion5()%></td>
						<td><%=contact.getRateIssue()%></td>
						<td><%=contact.getQualityIssue()%></td>
						<td><%=contact.getServiceIssue()%></td>
          				<td><%=contact.getExistingDealerCompany()%></td>
					<%
				}
				%>
        </tbody>
